...
<x-card>
    <div class="mb-6 pb-4 border-b border-zinc-100 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-4 gap-4 flex-1">
            <div>
        ...
        <form action="{{ url('/activity-log/hapus-semua') }}" method="POST" onsubmit="return confirmDeleteForm(event, 'Semua log aktivitas pada cabang aktif akan dihapus!')">
            @csrf
            @method('DELETE')
            <x-btn type="submit" variant="danger" icon="trash">Hapus Semua Log</x-btn>
        </form>
    </div>
